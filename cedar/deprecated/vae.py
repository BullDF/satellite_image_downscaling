...
class Encoder(nn.Module):
    def __init__(self):
        ...
        self.fc1 = nn.Linear(embed_size * len(nums_categories) + lstm_hidden_size * 3 + 2, 512)
        self.batch_norm1 = nn.BatchNorm1d(512)

        self.fc2 = nn.Linear(512, 256)
        self.batch_norm2 = nn.BatchNorm1d(256)

        self.out = nn.Linear(256, 2 * latent_dim)
...
class CalibrationVAE(nn.Module):
    ...
    def forward(self, x):
        ...
        mean, log_std = torch.chunk(z_params, 2, dim=1)
        log_std = torch.clamp(log_std, max=10)
        std = torch.exp(log_std)
        epsilon = torch.randn(mean.size(), device=x.device)
        z = mean + std * epsilon

        gamma_params = self.decoder(z)
        log_concentration, log_rate = torch.chunk(gamma_params, 2, dim=1)
        log_concentration = torch.clamp(log_concentration, max=10)
        log_rate = torch.clamp(log_rate, max=10)

        return z, mean, log_std, log_concentration, log_rate
